...
import { debounceTime, distinctUntilChanged, filter, tap, switchMap, catchError, map } from 'rxjs/operators';
...
export class TagsControlComponent implements OnDestroy, ControlValueAccessor {
  ...
  searchTags = (text$: Observable<string>) => text$.pipe(
    debounceTime(300),
    distinctUntilChanged(),
    filter((term) => term.length >= 2),
    tap(() => this.searching = true),
    switchMap(term => this.tagService.getTags(term).pipe(
      ...
      map((tags) => tags.filter(tag => !this.tags.includes(tag)).slice(0,10).map(tag => tag.name)),
